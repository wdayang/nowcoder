...
//3.当i表示百位，且百位对应的数为0,如n=31056(此时相当于30200),i=100，则a=310,b=56，此时百位为1的次数有a/10=31（最高两位0~30）
//综合:以上三种情况，当百位对应0或>=2时，有(a+8)/10次包含所有100个点，还有当百位为1(a%10==1)，
// 需要增加局部点b+1
...
public:
    ...
    {
        int num = 0;
        for(int i = 1; i <= n; i *= 10)   //按照各位十位百位进行计数
        {
            int a = n / i, b = n % i;
            //之所以补8，是因为当百位为0，则a/10==(a+8)/10，当百位>=2，补8会产生进位位，效果等同于(a/10+1)
